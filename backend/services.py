...
from helpers import normalize_ingredient_name
...
def get_food_by_name(food_name):
  ...
  if not foods:
    normalized = normalize_ingredient_name(food_name)
    cursor.execute("SELECT * FROM foods WHERE LOWER(name) LIKE LOWER(?)", (f"%{normalized}%",))
    foods = cursor.fetchall()
...
def get_ingredient_by_name(ingredient_name):
  ...
  cursor.execute("SELECT * FROM ingredients WHERE LOWER(name) = LOWER(?)", (normalized,))

  ingredients = [dict(row) for row in cursor.fetchall()]
  conn.close()
  return ingredients

def get_ingredients_for_food(food_name: str):
  """Get all ingredients and their amounts for a specific food by its name."""
  conn = get_db()
  cursor = conn.cursor()
